...
if __name__ == "__main__":
    ...
    gateway.entry_point\
      .setParameters('--schemaType MARC21 --marcxml --defaultRecordType BOOKS marc.xml.gz')
    gateway.entry_point.start()
    gateway.shutdown()
```

This is an event-driven approach. The Java PythonGateway Java class accepts a class that
implements the MinimalProcessor interface (events happens during the reading of input data),
and [generic parameters](usage.html#general-parameters). The process calls the Processor's
implemented methods when some event happens: opening or closing a file, before and after the
iteration etc. Here only one important event is really implemented: `processRecord` that
has two parameters: marcRecord and recordNumber. The later is simply a count number, the first
one is a [BibliographicRecord](./apidocs/de/gwdg/metadataqa/marc/dao/record/BibliographicRecord.html)
object that represents a bibliographical record (MARC21, PICA or UNIMARC). Another important
class is [DataField](./apidocs/de/gwdg/metadataqa/marc/dao/DataField.html) representing a data field
