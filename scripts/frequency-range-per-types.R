...
create_plot <- function(dir, name, count, field, .type) {
...
create_full_picture <- function(dir, name, abbreviation, field) {
  p1 <- create_plot(name, 1282172, field, "Books")
  p2 <- create_plot(name, 1282172, field, "Computer Files")
  p3 <- create_plot(name, 1282172, field, "Continuing Resources")
  p4 <- create_plot(name, 1282172, field, "Maps")
  p5 <- create_plot(name, 1282172, field, "Music")
  p6 <- create_plot(name, 1282172, field, "Visual Materials")
...
create_all_pictures <- function(dir, name, field) {
  print(paste('create_all_pictures:', dir))
  df <- read_csv(sprintf('%s/%s', dir, file), col_types = col_types)
  df_summary <- df %>% 
    group_by(type) %>% 
    summarise(count = n()) %>% 
  ...
  for (i in 1:nr_rows) {
    ...
    picture <- create_plot(dir, name, 1282172, field, row$type)
    ...
    ggsave(
      sprintf(
        '%s/frequency-explained-by-type-%s-%s.png',
        img_dir, name, row$machine_name
      ),
